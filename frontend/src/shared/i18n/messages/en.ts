export default {
  common: {
    appName: "Nikki",
    loading: "Working",
    retry: "Retry",
    delete: "Delete",
    edit: "Edit",
    clearFilters: "Clear filters",
    untitled: "Untitled",
    previousDay: "Previous day",
    nextDay: "Next day",
    today: "Today",
    close: "Close",
    remove: "Remove",
    removeNamed: "Remove {name}",
    keyboardShortcuts: "Keyboard shortcuts",
    focusMode: "Focus mode",
    exitFocusMode: "Exit focus",
    entryActions: "Diary actions",
    saveBeforeExport: "You can export after saving.",
    exportMarkdown: "Export Markdown",
    offline:
      "You are offline. Saving and sync can retry when connection returns.",
  },
  auth: {
    panelLabel: "Authentication",
    modeLabel: "Authentication mode",
    login: "Log in",
    signup: "Sign up",
    signupHeading: "Create account",
    setupHeading: "Create first account",
    loginSubtitle: "Log in to Nikki.",
    signupSubtitle: "Start using Nikki.",
    setupSubtitle: "Create the first account for this Nikki.",
    signupAction: "Create account",
    setupAction: "Create first account",
    username: "Username",
    usernamePlaceholder: "username",
    password: "Password",
    passwordPlaceholder: "8+ characters",
    showPassword: "Show password",
    hidePassword: "Hide password",
    show: "Show",
    hide: "Hide",
    signupNote: "Sign up is available only when enabled in settings.",
    setupNote:
      "After this account is created, first-run setup signup closes automatically.",
  },
  setup: {
    panelLabel: "Initial setup",
    heading: "Welcome to Nikki",
    lead: "This Nikki instance has not been initialized. Create the first owner account or restore an operational backup archive.",
    publicWarning:
      "If this instance is already public, do not continue unless you are the operator.",
    tokenHelp:
      "The setup token is the value of NIKKI_FIRST_USER_BOOTSTRAP_TOKEN set by the operator before starting Nikki.",
    token: "Setup token",
    startFreshTitle: "Start fresh",
    startFreshBody: "Create the first owner account for this empty Nikki.",
    restoreTitle: "Restore from backup",
    restoreBody:
      "Upload a Nikki operational backup archive. Content backup zip files are not accepted here.",
    backupFile: "Backup file",
    backupFilePlaceholder: "No file selected",
    chooseBackupFile: "Choose file",
    verifyBackup: "Verify backup",
    restoreBackup: "Restore",
    restoreVerified: "Backup verified",
    backupCreatedAt: "Created",
    nikkiVersion: "Nikki version",
    schemaVersion: "Schema version",
    entryCount: "Entries",
    imageCount: "Images",
    backupSize: "Backup size",
    restoreConfirm:
      "Restore this backup into the empty Nikki instance and close setup after success.",
    restoreMissingInput: "Enter the setup token and choose a backup file.",
    restoreVerifyFirst: "Verify the backup before restoring.",
    restoreConfirmRequired: "Confirm the restore before continuing.",
    passwordConfirmation: "Password confirmation",
    createOwner: "Create owner account",
    passwordMismatch: "Password confirmation does not match.",
    locked: "Setup is already complete.",
  },
  nav: {
    main: "Main",
    today: "Today",
    calendar: "Calendar",
    entries: "Diary",
    settings: "Settings",
    signedIn: "Signed in",
    entriesCount: "Entries",
    streak: "Streak",
    logout: "Log out",
  },
  entries: {
    diary: "Diary",
    list: "List",
    archive: "Archive",
    viewMode: "Diary view mode",
    jumpLabel: "Jump to diary date",
    jumpToDate: "Jump to date",
    jump: "Jump",
    search: "Search",
    searchPastEntries: "Search past entries",
    searchFilters: "Search filters",
    activeDiaryFilters: "Active diary filters",
    diaryEntries: "Diary entries",
    mood: "Mood",
    tag: "Tag",
    from: "From",
    to: "To",
    exactTag: "Exact tag",
    hasImage: "Has image",
    activeSearchFilters: "Active search filters",
    searchHint:
      "Search title, body, mood, and tag text. Use filters for exact mood, exact tag, date, or image matches.",
    searching: "Searching...",
    searchFailed: "Search failed",
    searchFailedRetry: "Search failed. Try again in a moment.",
    matchingEntries: "{count} matching entry | {count} matching entries",
    noSearchMatches: "No entries match this search.",
    noSearchMatchesDetail:
      "Try a different word, widen the date range, or remove an exact filter.",
    removeFilters: "Remove filters",
    filterSearch: "Search: {query}",
    filterMood: "Mood: {mood}",
    filterTag: "Tag: {tag}",
    filterFrom: "From {date}",
    filterTo: "To {date}",
    filterExactTag: "Exact tag: {tag}",
    noEntriesMatchFilters: "No entries match the active filters",
    noEntriesAvailable: "No entries available",
    noEntriesMatchCurrentFilters: "No entries match current filters",
    noEntriesYet: "No entries yet",
    clearFiltersDetail:
      "Clear filters or try a different search to see more diary entries.",
    entriesAppearDetail:
      "Diary entries will appear here after they are created.",
    showingEntries: "Showing {visible} of {total} entries",
    loadMore: "Load more",
    archiveMonths: "Archive months",
    archiveMonth: "Archive month",
    archiveMonthCalendar: "{month} calendar",
    entriesCount: "{count} entries",
    noEntriesInMonth: "No entries in this month for the current filters.",
    dateNavigation: "Diary date navigation",
    noBody: "No body",
    titlePlaceholder: "Untitled",
    bodyLabel: "Body",
    bodyPlaceholder: "How was today?",
    textStyleMenu: "Text formatting menu",
    paragraph: "Body",
    heading1: "Heading 1",
    heading2: "Heading 2",
    bold: "Bold",
    italic: "Italic",
    strike: "Strikethrough",
    link: "Link",
    quote: "Quote",
    bulletList: "Bulleted list",
    orderedList: "Numbered list",
    code: "Code",
    moodHappy: "Happy",
    moodCalm: "Calm",
    moodTired: "Tired",
    moodSad: "Blue",
    moodExcited: "Excited",
    saveDirty: "Unsaved",
    saveSaving: "Saving",
    saveSaved: "Saved",
    saveFailed: "Save failed",
    saveConflict: "Conflict",
    loadServerVersion: "Load server version",
    recoveryDraftHeading: "Unsaved local draft",
    recoveryConflictHelp:
      "Your local edit was not saved. Review or copy it before choosing what to keep.",
    recoveryServerLoadedHelp:
      "The server version is loaded. Your unsaved local draft is still kept below.",
    recoveryDraftLabel: "Recovered local draft",
    recoveryDate: "Date",
    recoveryTitle: "Title",
    recoveryMood: "Mood",
    recoveryTags: "Tags",
    recoveryBody: "Body",
    recoveryEmpty: "(empty)",
    copyRecoveryDraft: "Copy draft",
    recoveryCopied: "Copied",
    recoveryCopyFailed: "Copy failed",
    restoreRecoveryDraft: "Restore draft to editor",
    dismissRecoveryDraft: "Dismiss draft",
    navigationSaving: "Please wait for saving to finish before changing dates.",
    navigationRetryAfterSave: "Try {source} again after saving finishes.",
    navigationConflictBlocked:
      "Date navigation is paused until the conflict is resolved.",
    navigationFailedBlocked:
      "Date navigation is paused to protect content that failed to save.",
    uploadAborted: "Image upload was canceled.",
    conflictStaleVersion:
      "The server version has changed. Saving now may overwrite those changes.",
    conflictDateExists: "An entry already exists for that day.",
    sourceDateNavigation: "date navigation",
    sourceToday: "Today",
    sourceCalendar: "Calendar",
    sourceList: "diary list",
    sourceArchive: "archive",
    sourceSearch: "search",
    sourceMemory: "memory shelf",
    sourceEdit: "edit",
    returnToToday: "Back to Today",
    memoryEyebrow: "Memory",
    memoryTitle: "From the shelf",
    memoryCollapse: "Collapse memories",
    memoryExpand: "Expand memories",
    memoryLoading: "Looking through past entries...",
    memoryLoadFailed: "Memories could not be loaded.",
  },
  calendar: {
    previousMonth: "Previous month",
    nextMonth: "Next month",
    weekdays: ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"],
  },
  settings: {
    title: "Settings",
    account: "Account",
    export: "Export",
    language: "Language",
    memories: "Memories",
    memoriesEnabled: "Show random memories on Today",
    memoriesExcludedMoods: "Moods to hide",
    changePassword: "Change password",
    changePasswordBody:
      "Changing it signs out every session. Log in again after this completes.",
    currentPassword: "Current password",
    newPassword: "New password",
    confirmNewPassword: "Confirm new password",
    changePasswordCancel: "Cancel",
    changePasswordSubmit: "Change",
    changePasswordCurrentRequired: "Enter your current password.",
    changePasswordLength: "New passwords must be 8 to 200 characters.",
    changePasswordMismatch: "The new password confirmation does not match.",
    changePasswordSame: "Enter a password different from the current one.",
    deleteAccount: "Delete account",
    deleteAccountBody:
      "This permanently deletes your diary entries, images, and sessions. Export anything you need first.",
    deleteAccountUsernameConfirm: "Enter your username to confirm",
    deleteAccountPasswordConfirm: "Current password",
    deleteAccountCancel: "Cancel",
    deleteAccountSubmit: "Delete permanently",
    deleteAccountUsernameMismatch: "The username does not match.",
    deleteAccountPasswordRequired: "Enter your password.",
    securityHistory: "Security history",
    securityRefresh: "Refresh security history",
    securityLoading: "Loading...",
    securityEmpty: "No history yet.",
    securityOutcomeSucceeded: "Succeeded",
    securityOutcomeFailed: "Failed",
    securityActor: "Actor {actor}",
    securityRemoteIp: "IP {ip}",
    securityReason: "Reason {reason}",
    securityTarget: "Target {target}",
    securityMetadataValue: "Value",
    securityMetadataFormat: "Format",
    securityMetadataEntries: "Entries",
    securityMetadataImages: "Images",
    securityMetadataRemainingUsers: "Remaining users",
    securityMetadataBytes: "bytes",
    securityMetadataBackupSize: "backup bytes",
    auditEvents: {
      authSignupSucceeded: "Sign up",
      authSignupFailed: "Sign up failed",
      authLoginSucceeded: "Log in",
      authLoginFailed: "Log in failed",
      authLogoutSucceeded: "Log out",
      authPasswordChanged: "Password changed",
      authPasswordChangeFailed: "Password change failed",
      authAccountDeleted: "Account deleted",
      authAccountDeleteFailed: "Account deletion failed",
      authCsrfFailed: "CSRF check failed",
      authRateLimited: "Authentication rate limited",
      setupOwnerCreated: "Owner created",
      setupOwnerCreateFailed: "Owner creation failed",
      setupRestoreVerified: "Restore backup verified",
      setupRestoreCompleted: "Restore completed",
      setupRestoreFailed: "Restore failed",
      exportCompleted: "Export",
      exportEntryMarkdownCompleted: "Entry Markdown export",
      entriesDeleted: "Entry deleted",
      imagesDeleted: "Image deleted",
    },
  },
  images: {
    photo: "Photo",
    addPhotos: "Add photos",
    preparing: "Preparing",
    uploading: "Uploading {progress}%",
    uploadFailed: "Upload failed",
    maxSize: "Images must be 8 MB or smaller.",
    fallbackUploadFailed: "Image upload failed.",
    missingAlt: "Attached image",
    missingHeading: "Image could not be loaded",
    missingHelp:
      "Check uploaded files in the backup and run cleanup-images --dry-run to find mismatches.",
    imageId: "Image ID",
    file: "File",
    url: "URL",
    entryDate: "Entry date",
    images: "Images",
  },
  export: {
    json: "JSON",
    markdown: "Markdown",
    backup: "Backup",
  },
  errors: {
    api: "An API error occurred.",
    generic: "An error occurred.",
    offline: "You are offline. Try again when the connection returns.",
    invalidDate: "Enter a valid date.",
    backend: {
      request: {
        too_large: "The request is too large.",
        invalid_json: "Check the request JSON.",
        invalid_id: "Check the ID.",
      },
      auth: {
        invalid_input: "Check the username and password.",
        invalid_credentials: "Check the username or password.",
        unauthorized: "Log in to continue.",
        password_unchanged: "Enter a password different from the current one.",
        signup_closed: "Sign up is currently disabled.",
        username_exists: "This username is already in use.",
        owner_account_required:
          "The owner account cannot be deleted while other users remain.",
        owner_required: "Log in with an owner account.",
        csrf: "Check the request and try again.",
        rate_limited: "Too many authentication requests. Try again later.",
      },
      setup: {
        invalid_input: "Check the setup form.",
        invalid_token: "Check the setup token.",
        invalid_backup: "Check the backup archive.",
        already_initialized: "Setup is already complete.",
        restore_failed: "Restore failed. Check the backup and try again.",
        restore_in_progress: "Setup restore is in progress.",
        username_exists: "This username is already in use.",
      },
      entries: {
        invalid_cursor: "Check the page cursor.",
        invalid_input: "Check the diary input.",
        not_found: "Entry not found.",
        date_exists: "An entry already exists for that day.",
        stale_version: "The entry was updated in another session.",
      },
      images: {
        invalid_image: "Check the image file.",
        too_large: "Images must be 8 MB or smaller.",
        quota_exceeded: "Image storage quota exceeded.",
        too_many: "Each entry can have up to 3 images.",
        not_found: "Image not found.",
        invalid_image_id: "Check the image ID.",
      },
      export: {
        unsupported_format: "Check the export format.",
        too_large: "Too many items to export.",
      },
      server: {
        internal: "Something went wrong on the server.",
        panic: "Something went wrong on the server.",
      },
    },
  },
};
