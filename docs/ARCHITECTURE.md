...
Entry retrieval APIs are authenticated and user-scoped:
...
- `GET /api/entries/search` supports basic diary search and filters for query text, date range, mood, tag, and image presence.
- `GET /api/entries/memories` supports the Today screen memory shelf. It requires a target `date`, only returns entries before that date, excludes empty entries that have no title/body/image, supports `excludeMoods`, caps `limit` at 12, and orders results deterministically for the target date. Results remain scoped to the signed-in user.

The memory shelf preferences in the frontend are browser-local display preferences. They do not change stored diary data, entry moods, or server-side ownership checks.
...
App-level JSON/Markdown export and app-level backup archive generation have guardrails to avoid unbounded memory growth: JSON/Markdown export is limited to 5,000 entries, and backup archive creation is limited to 5,000 entries and 10,000 images. Operational backup uses the Nikki operational backup archive.

Operational backup archive has these required parts:

- `manifest.json`.
- PostgreSQL custom-format dump at `db/postgres.dump`.
- Uploads archive at `uploads/uploads.tar`.
- Optional `SHA256SUMS`.
